...
@interface JLYAppServiceManager ()

@property (nonatomic, strong) Class<UIApplicationDelegate> appDelegateClass;
@property (nonatomic, strong) NSMutableDictionary<NSString*, id<JLYAppService>> *servicesMap;

@end
// 由子类实现服务代理方法，父类负责把这些实现添加到 ApplicationDelegate 中
// 这些都需要在 ApplicationDelegate 实例创建之前执行
@implementation JLYAppServiceManager

+ (instancetype)sharedManager {
    static JLYAppServiceManager *_sharedManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedManager = [JLYAppServiceManager new];
    });
...
- (void)registerService:(id<JLYAppService>)service {
    ...
    id<JLYAppService> pre = self.servicesMap[[service serviceName]];
    if (pre) {
...
- (BOOL)proxyCanResponseToSelector:(SEL)aSelector {
    ...
    [self.servicesMap enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, id<JLYAppService> _Nonnull obj, BOOL * _Nonnull stop) {
