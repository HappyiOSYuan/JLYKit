...
- (NSString *)jly_uuid{
    ...
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self jly_createUUID] forKey:key];
...
- (NSString *) jly_udid{
    NSString *udid = [[JLYUDIDGenerator sharedInstance] UDID];
    if (udid.length==0) {
        udid = [self jly_uuid];
        [[JLYUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *) jly_macaddressMD5{
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] jly_macaddress];
        macid = [macaddress jly_md5];
        if (!macid){
