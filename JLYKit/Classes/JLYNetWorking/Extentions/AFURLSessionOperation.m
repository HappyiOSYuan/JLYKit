...
- (instancetype)uploadOperationWithManager:(__kindof AFURLSessionManager *)manager
    ...
    operation.task = [manager uploadTaskWithRequest:request
                                           fromFile:fileURL
...
+ (instancetype)uploadOperationWithManager:(__kindof AFURLSessionManager *)manager
    ...
    operation.task = [manager uploadTaskWithStreamedRequest:request
                                                   progress:uploadProgressBlock
                                          completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                                              if (completionHandler) completionHandler(response, responseObject, error);
                                              [operation completeOperation];
...
+ (instancetype)downloadOperationWithManager:(__kindof AFURLSessionManager *)manager
    ...
    operation.task = [manager downloadTaskWithRequest:request
...
+ (instancetype)downloadOperationWithManager:(__kindof AFURLSessionManager *)manager
    ...
    operation.task = [manager downloadTaskWithResumeData:resumeData
                                                progress:downloadProgressBlock
                                             destination:destination
                                       completionHandler:^(NSURLResponse * _Nonnull response, NSURL * _Nullable filePath, NSError * _Nullable error) {
                                           if (completionHandler) completionHandler(response, filePath, error);
